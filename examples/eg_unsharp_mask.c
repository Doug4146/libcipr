...
int main(void)
{
    ...
    // Apply the unsharp mask filter to the image with amount=5 and stddev=0.75
    printf("Applying unsharp mask...\n");
    if (cipr_filter_unsharp_mask(image, 5, 0.75) != 0) {
        cipr_image_destroy(&image);
        cipr_thread_pool_shutdown();
    ...
    if (cipr_io_write_jpeg(image, "../examples/output_images/unsharp_mask.jpeg", 95) != 0) {
        cipr_image_destroy(&image);
        cipr_thread_pool_shutdown();
